...
public class SkillClassScreen extends ThemedPanelScreen {
    ...
    protected void buildContent(ScrollVBox content) {
        ...
        long currentGameTick = mc.level == null ? 0L : mc.level.getGameTime();
        ...
        for (int i = 0; i < skills.size(); i++) {
            addSkillRow(content, skills.get(i), i, skillData, availablePoints, currentGameTick, actionBtnW, slotBtnW);
        }
    }

    private void addSkillRow(ScrollVBox content, SkillDefinition skill, int index, IPlayerSkillData skillData,
                             int availablePoints, long currentGameTick, int actionBtnW, int slotBtnW) {
        int currentLevel = skillData.getSkillLevel(skill.getId());
        int maxLevel = skill.getMaxLevel();
        int remainingCooldown = skillData.getRemainingCooldownTicks(skill.getId(), currentGameTick);

        boolean hasPrerequisites = true;
        for (SkillPrerequisite prerequisite : skill.getPrerequisites()) {
        ...
        boolean canUpgrade = currentLevel < maxLevel && hasPrerequisites && availablePoints > 0;
        boolean canUseSkill = currentLevel > 0 && remainingCooldown <= 0;

        String cooldownText = remainingCooldown > 0 ? String.format("冷卻: %.1fs", remainingCooldown / 20.0f) : "冷卻: 就緒";
        String statusText = currentLevel >= maxLevel
                ? "已滿級"
                : (hasPrerequisites ? (availablePoints > 0 ? "可升級" : "點數不足") : "前置不足");

        HBox row = new HBox(0, 0, 0, ROW_H, ROW_GAP, 4, 4);
        row.addFill(new ScaledLabel(220, 18,
        ...
        content.addChild(new ScaledLabel(0, 14, Component.literal("  " + skill.getDescription()), theme.labelText()));

        ThemedButton castBtn = row.addRight(new ThemedButton(actionBtnW, 24, Component.literal("施放"), theme,
                b -> SkillPackets.requestCast(skill.getId())));
        castBtn.active = canUseSkill;

        ThemedButton upgradeBtn = row.addRight(new ThemedButton(actionBtnW, 24, Component.literal("升級"), theme, b -> {
        ...
        String keyLabel = index < SLOT_KEYS.length
                ...
                : "-";
        row.addRight(new ThemedButton(slotBtnW, 24, Component.literal("槽位 " + (index + 1) + "  鍵:" + keyLabel), theme,
                b -> {
                    if (minecraft != null) {
        ...
        if (!hasPrerequisites) {
            content.addChild(new ScaledLabel(0, INFO_H,
                    Component.literal("  前置未滿足，需先提升前序技能"),
                    theme.labelText()));
        }
    }
}
